...
from apps.core.models import UnidadesDeMedida, CategoriasMateriaPrima, CategoriasProductosReventa
...
class ProductosElaborados(models.Model):
    ...
    unidad_medida_nominal = models.ForeignKey(UnidadesDeMedida, on_delete=models.CASCADE, null=False, blank=False) 
    ...
    unidad_nominal_venta = models.DecimalField(max_digits=10, decimal_places=2, default=0)
    ...
    class Meta:
        constraints = [
            models.CheckConstraint(
                check=(Q(es_intermediario=True) & Q(punto_reorden__isnull=False)) |
                    (Q(es_intermediario=False) & Q(punto_reorden__isnull=True)),
