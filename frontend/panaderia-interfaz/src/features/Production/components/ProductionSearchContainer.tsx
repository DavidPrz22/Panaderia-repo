...
export const ProductSearchContainer = ({data, onSelection}: searchContainerProp) => {
    ...
    const { searchQuery, productSearchRef, setProductoId, showSearch, setShowSearch } = useProductionContext();
    useEffect(() => {
    ...
    return (
        <div className={`absolute top-[107%] left-0 max-h-[400px] cursor-pointer w-full overflow-auto bg-white border border-gray-300 border-b-0 rounded-md shadow-md z-10 transition-[opacity, transform] origin-top duration-200 ${showSearch ? 'opacity-100 scale-100' : 'opacity-0 scale-80'}`}>
                {
                filteredData.length > 0 ? (
                    filteredData.map((producto, index) => (
                        <ProductSearchItem key={index} product={producto.nombre_producto} id={producto.id} onClick={() => handleClick(producto)} />
                    ))
                ) : (
