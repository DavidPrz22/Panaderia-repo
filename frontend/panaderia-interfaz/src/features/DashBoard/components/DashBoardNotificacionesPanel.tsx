...
const NoNotification = () => (
  <div className=" text-muted-foreground text-md text-center font-medium py-10">
...
export function DashBoardNotificacionesPanel() {
  ...
  const [filtroTipoProducto, setFiltroTipoProducto] =
    useState<TipoProducto>(TODOS);
    ...
    if (!tipo_notificacion)
      return notificaciones.filter((n) =>
        filtroTipoProducto === TODOS
          ? true
          : n.tipo_producto === filtroTipoProducto,
      );
      ...
      <CardHeader>
        <CardTitle className="text-lg font-semibold flex items-center justify-between">
          ...
          <div className="flex items-center gap-4 justify-end">
            <div className="text-sm font-semibold">Filtrar por tipos:</div>
            <NotificacionesFiltro
              id="notificaciones-filtro"
              placeholder="Selecciona un tipo"
              value={filtroTipoProducto}
              onChange={(v) => setFiltroTipoProducto(v as TipoProducto)}
            />
          </div>
        </CardTitle>
      </CardHeader>
      <CardContent>
        <Tabs defaultValue="todos" className="w-full">
          <TabsList className="grid w-full grid-cols-5 h-10">
            ...
            <TabsTrigger value="nostock" className="cursor-pointer">
              Sin stock
            </TabsTrigger>
            <TabsTrigger value="expiracion" className="cursor-pointer">
              Vencimiento
            </TabsTrigger>
            <TabsTrigger value="entrega" className="cursor-pointer">
              Entregas
            </TabsTrigger>
          </TabsList>
          ...
          <TabsContent value="stock" className="mt-4">
            <NotificationList
              items={filterByType(NOTIFICACIONES_TIPOS.STOCK)}
            />
          </TabsContent>
          <TabsContent value="nostock" className="mt-4">
            <NotificationList
              items={filterByType(NOTIFICACIONES_TIPOS.SIN_STOCK)}
            />
          </TabsContent>
          <TabsContent value="expiracion" className="mt-4">
            <NotificationList
              items={filterByType(NOTIFICACIONES_TIPOS.VENCIMIENTO)}
            />
          </TabsContent>
          <TabsContent value="entrega" className="mt-4">
            <NotificationList
              items={filterByType(NOTIFICACIONES_TIPOS.ENTREGAS)}
            />
          </TabsContent>
        </Tabs>
      </CardContent>
