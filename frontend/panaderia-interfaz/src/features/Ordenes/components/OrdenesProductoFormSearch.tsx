...
export function OrdenesProductoFormSearch({
  value,
  onChange,
}: {
  value: string;
  onChange: (producto: OrdenProductoSearch) => void;
}) {
  const [open, setOpen] = useState(false);
  const [timer, setTimer] = useState<NodeJS.Timeout | null>(null);
  const { data: dataProductosPedido, mutate: searchProductosPedido } =
    useProductosPedidoSearchMutation();
          ...
          <CommandInput
            ...
            onValueChange={(e: string) => {
              ...
              const timerRef = setTimeout(
                () => searchProductosPedido(searchTerm),
                1000,
              );
              setTimer(timerRef);
            }}
