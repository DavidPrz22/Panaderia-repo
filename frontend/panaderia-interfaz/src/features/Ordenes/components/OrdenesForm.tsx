...
export const OrderForm = ({ order, onClose }: OrderFormProps) => {
  const { handleSubmit, watch, setValue } = useForm<TOrderSchema>({
    ...
    defaultValues: order
      ? {
          ...
          productos: order.productos.map((p) => ({
            producto: {
              id: Number(p.producto.id),
              tipo_producto: p.producto.tipo_producto!,
            },
  ...
  const { mutateAsync: createOrdenMutation, isPending: isCreatingOrden } =
    useCreateOrdenMutation();
  const { mutateAsync: updateOrdenMutation, isPending: isUpdatingOrden } =
    useUpdateOrdenMutation();
  ...
  const calculateTotalFromItems = (itemsArray: OrderLineItemForm[]) => {
    const subtotalBeforeFees = itemsArray.reduce(
      (sum, item) => sum + item.precio_unitario_usd * item.cantidad_solicitada,
      0,
    );
    ...
    const CantidadImpuesto = itemsArray.reduce((sum, item) => {
      const lineSubtotal = item.precio_unitario_usd * item.cantidad_solicitada;
      const lineSubtotalAfterDiscount =
        lineSubtotal * (1 - item.descuento_porcentaje / 100);
    ...
    setValue(
      "monto_total_usd",
      roundTo3(subtotalBeforeFees - CantidadDescuento + CantidadImpuesto),
    );
    setValue(
      "monto_total_ves",
      roundTo3(
        (subtotalBeforeFees - CantidadDescuento + CantidadImpuesto) *
          (Number(watch("tasa_cambio_aplicada")) || 0),
      ),
    );
  };
  ...
  const removeItem = (id: number, productoId: number) => {
    ...
    const newProductos =
      watch("productos")?.filter((p) => p.producto.id !== productoId) || [];
  ...
  const calculateSubtotal = (item: OrderLineItemForm) => {
    const subtotal =
      ...
      (1 + item.impuesto_porcentaje / 100);
    return roundTo3(subtotal);
  };

  const handleUpdate = (
    e: React.ChangeEvent<HTMLInputElement>,
    item: OrderLineItemForm,
    fieldSchema: keyof TOrderSchema["productos"][number],
    field: "descuento_porcentaje" | "impuesto_porcentaje",
  ) => {
    const productoId = item.producto.id!;
    const productoIndex = watch("productos")?.findIndex(
      (p) => p.producto.id === productoId,
    );
    if (productoIndex !== -1) {
      setValue(
        `productos.${productoIndex}.${fieldSchema}`,
          ...
          <CardContent className="space-y-6 pt-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              ...
              <div className="space-y-2">
                ...
                <OrdenesFormSelect
                  id="cliente"
                  value={watch("cliente") ? watch("cliente").toString() : ""}
                  onChange={(v) => {
              ...
              <div className="space-y-2">
                ...
                <OrdenesFormSelect
                  ...
                  value={
                    watch("estado_orden")
                      ? watch("estado_orden").toString()
                      : ""
                  }
                  placeholder="Selecciona un estado"
                  onChange={(v) => setValue("estado_orden", Number(v))}
                >
              ...
              <div className="space-y-2">
                ...
                <OrdenesFormSelect
                  ...
                  onChange={(v) => setValue("metodo_pago", Number(v))}
              ...
              <div className="space-y-2">
                ...
                <Input
                  ...
                  onChange={(e) =>
                    setValue("referencia_pago", e.target.value || undefined)
                  }
                    ...
                    {items.map((item) => (
                      <TableRow key={item.id + item.producto.id!}>
                        <TableCell>
                          <OrdenesProductoFormSearch
                            ...
                            onChange={(producto) => {
                              ...
                              setItems([...items]);
                              const schemaValue: TOrderSchema["productos"] =
                                items.map((item) => {
                                  return {
                                    producto: {
                                      id: item.producto.id!,
                                      tipo_producto:
                                        item.producto.tipo_producto!,
                                    },
                                    cantidad_solicitada:
                                      item.cantidad_solicitada,
                        ...
                        <TableCell>
                          <Input
                            ...
                            onChange={(e) => {
                              ...
                              const productoIndex = watch(
                                "productos",
                              )?.findIndex((p) => p.producto.id === productoId);
                              if (productoIndex !== -1) {
                                setValue(
                                  `productos.${productoIndex}.cantidad_solicitada`,
                                  Number(e.target.value),
                                  { shouldValidate: true },
                                );
                                item.cantidad_solicitada = Number(
                                  e.target.value,
                                );
                                const subtotal = calculateSubtotal(item);
                                item.subtotal = subtotal;
                        ...
                        <TableCell className="text-sm">
                          {item.unidad_medida_venta.abreviatura}
                        </TableCell>
                        <TableCell className="text-sm">
                        ...
                        <TableCell>
                          <Button
                            ...
                            size="icon"
                            onClick={() =>
                              removeItem(item.id, item.producto.id!)
                            }
                          >
                ...
                <div className="space-y-2 w-60">
                  <div className="flex justify-between">
                    ...
                    <span className="font-medium">
                      {bcvRate?.promedio ? bcvRate.promedio.toFixed(2) : "0.00"}
                    </span>
                  </div>
                  <div className="flex justify-between">
                    <span className="font-bold">Total en VES: </span>
                    <span className="font-medium">
                      {(Number(watch("monto_total_ves")) || 0).toFixed(2)}
                    </span>
                  </div>
                </div>
                ...
                <div className="space-y-2 w-80">
                  ...
                  <div className="flex justify-between">
                    ...
                    <span className="font-medium text-destructive">
                      -
                      {formatCurrency(
                        Number(watch("monto_descuento_usd")) || 0,
                      )}
                    </span>
                  </div>
                  <div className="flex justify-between">
                    ...
                    <span className="font-medium">
                      {formatCurrency(
                        Number(watch("monto_impuestos_usd")) || 0,
                      )}
                    </span>
                  </div>
                  <div className="flex justify-between text-xl font-bold border-t pt-2">
                    <span>Total:</span>
                    <span>
                      {formatCurrency(Number(watch("monto_total_usd")) || 0)}
                    </span>
                  </div>
                </div>
              </div>
