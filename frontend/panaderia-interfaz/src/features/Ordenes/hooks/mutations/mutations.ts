...
export const useUpdateOrdenMutation = () => {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: ({ id, data }: { id: number; data: TOrderSchema }) =>
      updateOrden(id, data),
    onSuccess: (_, { id }) => {
      queryClient.invalidateQueries({
        queryKey: ordenesTableQueryOptions.queryKey,
      ...
      queryClient.invalidateQueries({
        queryKey: ordenesDetallesQueryOptions(id).queryKey,
      });
    },
    onError: (error) => {
...
export const useRegisterPaymentReferenceMutation = () => {
  ...
  return useMutation({
    ...
    }) => registerPaymentReference(id, referencia_pago),
    onSuccess: (_, { id }) => {
      queryClient.invalidateQueries({
        queryKey: ordenesTableQueryOptions.queryKey,
      });
