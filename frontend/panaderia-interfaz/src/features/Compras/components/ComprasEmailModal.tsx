                                ...
                                >
                                    {files.length > 0 ? 
                                        <div className="flex flex-col gap-2">
                                            {files.map((file) => (
                                                <span key={file.name} className="text-sm font-semibold border px-2 py-1 border-gray-300 rounded-md">
                                                    {file.name}
                                                </span>
                                            ))}
                                            <Input type="file" id="file" multiple className="hidden" ref={fileInputRef} onChange={handleFilesChange} />
                                        </div> 
                                        :  
                                        <>
                                            <FolderOpen className="w-4 h-4" />
                                            <span className="text-sm font-semibold">Seleccionar archivo</span>
                                            <Input type="file" id="file" multiple placeholder="Ingrese archivo adjunto" className="hidden" ref={fileInputRef} onChange={handleFilesChange} />
                                        </> 
                                    }
                                </Button>
                            </CardContent>
