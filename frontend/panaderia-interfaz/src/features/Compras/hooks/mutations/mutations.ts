...
import { searchProductosOC, createOrdenCompra, marcarEnviadaOC } from "../../api/api";
...
export const useMarcarEnviadaOCMutation = () => {
    ...
    return useMutation({
        ...
        onSuccess: (_, id) => {
            queryClient.invalidateQueries({ queryKey: ordenesCompraTableQueryOptions.queryKey });
            queryClient.invalidateQueries({ queryKey: ordenesCompraDetallesQueryOptions(id).queryKey });
        },
        onError: (error) => {
