...
export default function PIInputFormSearch({ setValue }: setValueProps) {
    const { searchList, searchTimer, setSearchTimer, setSearchList, recetaSearchInputRef } = useProductosIntermediosContext();
    ...
    return (
        <div id="receta-search-container" className="flex flex-col relative">
            <div className="relative">
                <input  ref={recetaSearchInputRef} 
                        onChange={handleChange} 
                        onFocus={handleFocus}
                        placeholder="Busca una receta para el producto" type="text" className="block w-full pl-3 pr-10 py-2.5 border border-gray-300 rounded-md shadow-xs font-[Roboto]
            ...
            {isFocused && (
                isPending ? (
                    <PendingTubeSpinner size={10} extraClass="absolute top-[100%] left-0 w-full h-full bg-white border border-gray-300 rounded-md shadow-md" />
                ) : completed && searchList.length === 0 ? (
                    <NoResults>Ninguna receta encontrada...</NoResults> 
                ) : (
                    searchList.length > 0 ? <RecetaSearchContainer searchList={searchList} setValue={setValue} /> : null
                )
            )}
        </div>
    );
